...
const mockCustomNotifications: Notifications[] = [
  {
    id: "1",
    active: true,
    name: "AWB arrived at airport",
    condition: {
      subject: "AWB Status",
      operator: "is",
      subject_state: "Arrived",
    },
    email_subject: "AWB [AWB_NUMBER] has arrived at [AWB_DESTINATION]",
    email_content:
      "AWB [AWB_NUMBER] has arrived at [AWB_DESTINATION] at [AWB_ARRIVAL_TIME]",
  },
]
...
export default function NotificationSettingsPage() {
  //   const form = useForm<{ customNotifications: Notifications[] }>({
  //     defaultValues: { customNotifications },
  //   })
  ...
  const [conditionOperatorOptions, conditionStateOptions] = useMemo(() => {
    ...
    const conditionSubject = conditionOptions.find(
      (item) => item.subjectName === selectedConditionSubject
    )
                  ...
                  <CardContent className="text-sm">
                    <div className="flex flex-row flex-wrap items-center gap-4">
                      ...
                      <InputSwitch<Notifications>
                        ...
                        type="combobox"
                        className="min-h-[20px] min-w-10 !border-0 border-zinc-900 bg-transparent p-0 text-sm hover:bg-transparent"
                        selectOptions={conditionSubjectOptions}
                      />
                      {form.watch("condition.subject") && (
                        <InputSwitch<Notifications>
                      ...
                      {form.watch("condition.operator") && (
                        <InputSwitch<Notifications>
                          name="condition.subject_state"
                          type="combobox"
                          className="min-h-[20px] min-w-10 !border-0 border-zinc-900 bg-transparent p-0 text-sm hover:bg-transparent"
                          selectOptions={conditionStateOptions}
                        />
                      )}
                    </div>
                  </CardContent>
