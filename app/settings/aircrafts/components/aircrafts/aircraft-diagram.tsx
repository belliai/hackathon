...
const AircraftDiagram = ({ ULDTotal, orientation = 'vertical' }: { ULDTotal: number, orientation?: 'vertical' | 'horizontal' }) => {
  ...
  return (
    <div className={`relative w-full h-full bg-zinc-900/50 custom-scrollbar ${orientation === 'vertical' ? 'overflow-y-auto overflow-x-hidden' : 'overflow-x-auto overflow-y-hidden'}`}>
      <div className={`sticky ${orientation === 'vertical' ? 'top-[30vh]' : 'top-[10vh]'} left-4 flex flex-col z-[2]`}>
        <Tooltip>
...
const HorizontalModel = ({ zoom, data }: { zoom: number, data: CargoData[] }) => (
  <div className={`${styles.airplaneContainerHorizontal} flex-row h-[200px]`} style={{ transform: `scale(${zoom})`, transformOrigin: 'center' }}>
    ...
    <div className={`${styles.airplaneHeadHorizontal} w-[600px] h-[175px] border-b-0 bg-[#4D4D4D] flex justify-center -mt-[150px]`}>          
    </div>

    <div className='p-4 min-w-[500px] h-[175px]'></div>
