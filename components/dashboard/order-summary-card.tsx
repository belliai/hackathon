...
export default function OrderSummaryCard(props: any) {
  ...
  const bookingType =
    bookingTypes &&
    bookingTypes.find((item: any) => item.ID === booking_type_id)
  const partnerPrefix =
    partnerPrefixes &&
    partnerPrefixes.find((item: any) => item.ID === partner_prefix_id)
  const originLoc =
    origin_id &&
    locations &&
    locations.find((item: any) => item.ID === origin_id)
  const destLoc =
    destination_id &&
    locations &&
    locations.find((item: any) => item.ID === destination_id)
  const commodity =
    commodity_code_id &&
    commodityCodes &&
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4">
        ...
        <CardDescription>
          {partnerPrefix && partnerPrefix.name}-{awb}
        </CardDescription>
      </CardHeader>
      <Separator />
