  ...
  return (
    <div className="flex flex-col mt-5 gap-4">
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {CNOTE_TYPE_LIST.map((cnoteType) => (
                  <SelectItem key ={cnoteType.id} value={cnoteType.label}>{cnoteType.label}</SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
          </Select>
      ...
      <div className="flex gap-4">
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {AWB_TYPE_LIST.map((awbType) => (
                  <SelectItem key ={awbType.id} value={awbType.label}>{awbType.label}</SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
          </Select>
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {STOCK_TYPE_LIST.map((stockType) => (
                  <SelectItem key ={stockType.id} value={stockType.label}>{stockType.label}</SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
          </Select>
        ...
        <div className="w-2/12">
          <Input className="border-zinc-500" placeholder="AWB Number" rightIcon={<MagnifyingGlassIcon className="h-4 w-4 text-zinc-400" />} />
        </div>
            ...
            <SelectContent>
              <SelectGroup>
                {JOB_STATUS_LIST.map((jobStatus) => (
                  <SelectItem key ={jobStatus.id} value={jobStatus.label}>{jobStatus.label}</SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
          </Select>
