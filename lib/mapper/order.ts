...
const extractId = (nestedObj: any) => nestedObj ? nestedObj.ID || nestedObj.id : "";

const extractFormatDate = (date: Date, formatStr: string) => date ? format(date, formatStr) : null
...
export const mapJsonToSchema = (data: any) => {
    ...
    const shipperDetailsTransformed = data && data.shipper_details && data.shipper_details.map((detail: any) => ({
        ...
        date: detail.date && extractFormatDate(new Date(detail.date), "yyyy-MM-dd") || "",
    ...
    const mappedData = {
        ...
        created_at: data.created_at &&  extractFormatDate(new Date(data.created_at), "dd-MM-yyyy HH:mm") || "",
...
export const mapSchemaToJson = (data: any) => {
    ...
    const shipperDetailsTransformed = data && data.shipper_details && data.shipper_details.map((detail: any) => ({
        ...
        partner_type_id: detail.partner_type_id || null,
        partner_code_id: detail.partner_code_id || null,
        date: detail.date && extractFormatDate(new Date(detail.date), "yyyy-MM-dd") || "",
        flight_code: detail.flight_code || "",
        allotment_code: detail.allotment_code || "",
