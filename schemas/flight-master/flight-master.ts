...
export const flightMasterFormSchema = z.object({
    ...
    fromDate: z.date(),
    toDate: z.date(),
    frequencyItems: z.array(z.string()).nonempty({ message: "Please select at least 1 Day" }),
    ...
    }).partial().refine((data) => data.deptDay !== '' && data.deptHour !== '' && data.deptMinute !== '', {
    ...
    arrivalTime: z.object({
        ...
        arrivalMinute: z.string().min(1, { message: "Arrival Minute is required" }),
    }).partial().refine((data) => data.arrivalDay !== '' && data.arrivalHour !== '' && data.arrivalMinute !== '', {
